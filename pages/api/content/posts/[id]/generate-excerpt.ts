// Generate Excerpt API Route - /api/content/posts/[id]/generate-excerpt
// Uses OpenAI to generate an excerpt from the current post content
...
async function handler(
  ...
  try {
    ...
    console.log('Found post:', { id: post.id, title: post.title, contentType: typeof post.content });
    ...
    if (!plainText.trim()) {
      return res.status(400).json({
        ...
        error: {
          code: 'NO_CONTENT',
          message: 'Post has no content to generate excerpt from',
        },
      });
    }
